...
/**
 * Actuarial weighting coefficient update flyway generator
 *
 * This flyway generator takes 2 CSV's, one with CT offence codes and rsr coefficients.
 * Maps the codes to the coefficients from the CSV's.
 * Compares against DB coefficient values and identifies discrepancies.
 * Produces a versioned flyway script containing updates in src/main/resources/migration/common
 *
 * Running the generator:
 * - replace the string value of CT_OFFENCES_PATH to the path to the CSV
 * - replace the string value of RSR_COEFFICIENTS_UPDATE_PATH to the path to the CSV
 * - create a .env and set DB_URL, DB_USER, DB_PASSWORD for DB connection
 * - run main
 * - check src/main/resources/migration/common for newly generated update script
 */

// Path to CT offences extract
const val CT_OFFENCES_PATH = "path/to/CT_offences/CSV"

// Path to RSR coefficients extract
const val RSR_COEFFICIENTS_UPDATE_PATH = "path/to/RSR_coefficients/CSV"
